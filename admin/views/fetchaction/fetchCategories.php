...
if(isset($_POST) && isset($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest'){
  ...
  $results = $db_connection->query("SELECT COUNT(*) FROM category");

	$get_total_rows = $results->fetch_row();

	$total_pages = ceil($get_total_rows[0]/$item_per_page);
	$page_position = (($page_number-1) * $item_per_page);
  $results = $db_connection->prepare("SELECT CategoryID, CategoryName, Remark FROM category LIMIT $page_position, $item_per_page");

	$results->execute();
	$results->bind_result($c_id, $c_name, $remark);
	...
	echo "<table id='content-table' class='table table-striped table-hover'>
        ...
        if($page_number>1) $counter = ($item_per_page*($page_number-1)) + $counter;
      	while($results->fetch()){
            $counter++;
            $qty = get_product_qty($c_id);

      			echo "<tr>";
            echo "<td class='text-center'>$counter</td>";
           	echo "<td class='text-center'>000$c_id</td>";
            echo "<td class='text-center'>$c_name</td>";
            echo "<td class='text-center'>$remark</td>";
            echo "<td class='text-center'>$qty</td>";
            echo "<td>
                    <a href='editCategory.php?categoryID=$c_id'>Edit</a>
                  </td>";
            echo "<td>
                    <a href='deleteCategory.php?categoryID=$c_id'>Delete</a>
                  </td>";
            echo "</tr>";
      	}
	...
	function get_product_qty($cat_id){
    	$db_connection = new mysqli(DB_HOST, DB_USER, DB_PASS, DB_NAME);
      $result = $db_connection->query("SELECT COUNT(ProductID) FROM product WHERE CategoryID ='$cat_id'");
      $qty = $result->fetch_row();
      return $qty[0];
	}
